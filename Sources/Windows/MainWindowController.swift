...
final class MainWindowController: NSWindowController, NSWindowDelegate {
    ...
    init(model: MainWindowModel, store: MetricsStore, terminator: ProcessTerminator, settings: AppSettings, license: LicenseManager) {
        self.model = model
        ...
        window.contentView = NSHostingView(rootView: MainWindowView()
            .environment(model).environment(store).environment(terminator).environment(settings).environment(license))
